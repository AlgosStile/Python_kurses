...
# Вариант b думаю можно было бы рандомизировать, но пока не знаю как
# мозг уже кипит
...
def input_dat(name):
    x = int(input(
        f"{name}, введите количество конфет, которое возьмете от 1 до 28: "))
    while x < 1 or x > 28:
        x = int(input(f"{name}, введите корректное количество конфет: "))
    return x


def p_print(name, kas, counter, value_1):
    print(
        f"Ходил {name}, он взял {kas}, теперь у него {counter}. "
        f"Осталось на столе {value_1} конфет.")
