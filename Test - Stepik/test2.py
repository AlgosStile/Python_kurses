...
# x = input()
# y = "Цифр нет"  # Создаем переменную 'y' и записываем в неё 'Цифр нет'
# for i in range(len(str(x))):  # Создаем цикл 'i' равный длине строки
#     if x[i] in '0123456789':  # Создаем условие если x[i] содержит ('012..9')
#         y = "Цифра"  # y меняем на "Цифра"
...
# и 21 согласная буква (б, в, г, д, ......щ).

# s = input()
# son, cons = 0, 0  # назначаем переменные
# glas = 'ауоыиэяюёеАУОЫИЭЯЮЁЕ'  # переменная хранящая гласные
# sogl = 'бвгджзйклмнпрстфхцчшщБВГДЖЗЙКЛМНПРСТФХЦЧШЩ'  # переменная хранящая согласные
#
# for c in s:  # циклом проходим по строке
#     if c in glas:  # если символ соответствует одному из символов строки согласных
#         son += 1  # прибавляем к переменной согласные +1
#     if c in sogl:  # если символ соответствует одному из символов строки гласных
...
# счисления. Напишите программу, которая переводит данное число в двоичную
# систему счисления.
# Формат входных данных
# На вход программе подается одно натуральное число.
# Формат выходных данных
...
#     # При написанном в коде варианте, число в двоичной системе исчисления
#     # идёт в нужном порядке, если слагаемые поменять местами так
#     # d = d + str(n % 2), то двоичное число окажется записано наоборот, с конца.
#     # n //= 2
# print(d)
...
"""
# s = input()
# p: str = s[::-1]  # переворачиваем строку s
# if s == p:  # если строка "s" равно перевернутой строке "p"
#     print("YES")
# else:
#     print("NO")
# ++++++++++++++++++++++++++++++++
...
которая разрежет ее на две равные части, 
...
# a = x // 2 + x % 2 # определяем индекс элемента, с которого начинается часть
# # строки после разреза
# print(s[a:] + s[:a]) # склеиваем части строки с этого элемента до конца
# # и с начала до этого элемента (не включительно)
