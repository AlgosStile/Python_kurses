...
CHOICE, RATIONAL_ONE, RATIONAL_TWO, OPERATIONS_RATIONAL, OPERATIONS_COMPLEX, COMPLEX_ONE, COMPLEX_TWO = range(
    7)
    ...
    if user_choice in '123':
        ...
        if user_choice == '2':
            context.bot.send_message(update.effective_chat.id,
                                     'Введите Re и Im первого числа через ПРОБЕЛ: ')
            return COMPLEX_ONE
        if user_choice == '3':
...
def rational_one(update, context):
    user = update.message.from_user
    logger.info("Пользователь ввел число: %s: %s", user.first_name,
                update.message.text)
...
def operatons_rational(update, context):
    user = update.message.from_user
    logger.info(
        "Пользователь выбрал операцию %s: %s", user.first_name,
        update.message.text)
...
def complex_one(update, context):
    ...
    if ' ' in test and (test.replace(' ', '')).isdigit():
        ...
        update.message.reply_text(
            'Ошибка ввода. Введите Re и Im первого числа через ПРОБЕЛ')
...
def complex_two(update, context):
    ...
    if ' ' in test and (test.replace(' ', '')).isdigit():
        ...
        update.message.reply_text(
            'Ошибка ввода. Введите Re и Im второго числа через ПРОБЕЛ')
...
if __name__ == '__main__':
    ...
    conversation_handler = ConversationHandler(
        ...
        states={
            ...
            OPERATIONS_RATIONAL: [
                MessageHandler(Filters.text, operatons_rational)],
            OPERATIONS_COMPLEX: [
                MessageHandler(Filters.text, operatons_complex)],
